...
public class ButtonCreation {
	...
	public static void createRealItemRow(int buttonID) {
		...
		for (Item item : itemsList) {
			...
			if (itemCount < 16) {
				...
				if (itemCount % 4 == 3 || itemCount == itemsList.size() - 1) {
					addToButtonView(itemRows[itemCount / 4], 0, 7 + itemCount / 4);
					itemRows[itemCount / 4] = new HBox();
					itemRows[itemCount / 4].setAlignment(Pos.TOP_CENTER);
					itemRows[itemCount / 4].setSpacing(10);
				}
	...
	public static void addToButtonView(HBox node, int columnIndex, int rowIndex) {
		if (buttonView != null) {
